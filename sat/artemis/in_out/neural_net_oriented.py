...
from ..utils.vocabulary import Vocabulary, HuggingfaceVocab
...
def pass_artemis_splits_to_datasets(df, load_imgs, img_transforms, top_img_dir, n_emotions=9, one_hot_emo=True):
    ...
    for split, g in df.groupby('split'):
        ...
        # n_languages = len(g.language_label.unique())
        n_languages = 4

        dataset = AffectiveCaptionDataset(img_files, g.tokens_encoded, g.emotion_label, g.language_label,
                                          n_languages=n_languages, n_emotions=n_emotions, 
