...
class AffectiveCaptionDataset(Dataset):
    ...
    """
    def __init__(self, image_files, tokens, emotions, languages, n_languages, n_emotions=9, img_transform=None, one_hot_emo=True):
    ...
    def __getitem__(self, index):
        ...
        if self.n_languages > 0:
            if self.one_hot_emo:
                language = np.zeros(self.n_languages, dtype=np.float32)
                language[self.languages[index]] = 1
            else:
                language = self.languages[index]
        else:
            language = []

        res = {'image': img, 'emotion': emotion, 'language':language, 'tokens': text, 'index': index}
        return res
...
def custom_grounding_dataset_similar_to_affective_loader(grounding_data_csv, loader, n_workers=None):
    ...
    if 'language_label' in df.columns:
        languages = df.language_label
    else:
        languages = dummy

    standard_dset = loader.dataset
    custom_dataset = AffectiveCaptionDataset(image_files, dummy, emotions=emotions, languages=languages,
                                             n_languages= 4, n_emotions=standard_dset.n_emotions,
