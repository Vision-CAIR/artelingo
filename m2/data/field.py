...
class TextField(RawField):
    ...
    def process(self, batch, device=None):
        ...
        # pdb.set_trace()
        tensor = self.vocab.tokenize(list(batch), self.max_len)
        return tensor
    ...
    def decode(self, word_idxs, join_words=True):
        ...
        captions = self.vocab.batch_decode_print(word_idxs)
        # captions = []
...
class ArtEmisDetectionsField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=100):
        ...
        with open(os.path.join(detections_path,'wikiart_split.pkl'),'rb') as file:
            self.paints_ids_dict = {}
            img_id_list = pickle.load(file)
            for k, v in img_id_list:
                self.paints_ids_dict[unicodedata.normalize('NFD', k)] = v
    ...
    def preprocess(self, x):
        ...
        id = self.paints_ids_dict[unicodedata.normalize('NFD', x)]
...
class LanguageField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, languages=None):
        
        self.language_mapping = { key: value for value, key in enumerate(languages)}
        super(LanguageField, self).__init__(preprocessing, postprocessing)

    def preprocess(self, x):
        return self.language_mapping[x]